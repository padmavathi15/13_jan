...
const MonthlyReport = () => {
  ...
  const handleShowReport = () => {
    ...
    showSwal(
      'Generate Report?',
      'Do you want to generate the monthly report?',
      'question',
      () => {
        setError('');
        setValidated(false);
  ...
  const handleSendReport = () => {
    showSwal(
      'Are you sure?',
      'Do you want to send the monthly report?',
      'question',
      () => {
        Swal.fire('Sent!', 'Monthly Report sent successfully!', 'success');
      }
    );
  };
  ...
  return (
    <div style={{ backgroundColor: '#f5f7ff', padding: '30px', height: '100%', borderRadius: '15px' }}>
      <h4 className="text-center mb-4">Monthly Report</h4>
      ...
      <Modal show={showAddMarksModal} onHide={() => setShowAddMarksModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Add Marks for {selectedSubject}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
